...
public class MavenLicenseScanner implements Scanner {
    ...
    public Set<Dependency> scan(SensorContext context) {
        ...
        if (!licenseReport.exists()) {
            LOGGER.info(
                String.format(
                    "No licenses.xml file found in %s - skipping Maven license scan",
                    licenseReport
                )
            );
        }
        ...
        try {
            ...
            return LicenseSummaryReader
                .parseLicenseSummary(licenseReport)
                .stream()
                .map(projectLicenseInfo -> {
                    String dependencyName = String.format(
                        "%s:%s",
                        projectLicenseInfo.getGroupId(),
                        projectLicenseInfo.getArtifactId()
                    );

                    // use first dependency only
                    // mirrors the behaviour of MavenDependencyScanner
                    String licenseName = projectLicenseInfo
                        .getLicenses()
                        .stream()
            ...
            LOGGER.warn(
                String.format(
                    "Could not parse dependencies from license summary at \"%s\"!",
                    licenseReport
                ),
                e
            );
